...
2. Validate case templates and external paths:
   `femic prep validate-case --run-config config/run_profile.mkrf.yaml`
...
8. Add/edit `config/tipsy/tsamkrf.yaml`.
9. Run compile workflow from this instance root once the real MKRF boundary,
   source data, and runtime choices are ready.
